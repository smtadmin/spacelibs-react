...
class EZForm extends React.Component {
	...
	render() {
		...
		if (this.state.status === EZFormStatus.loading) {
			...
			output = (
				<div>
					404 Couldn&apos;t find form {this.props.formId}
				</div>
				);
		...
		return (
			<>
				{output}
				{(this.state.status === EZFormStatus.inProgress || this.state.status === EZFormStatus.submitting) && 
					<div style={{display: this.state.status === EZFormStatus.inProgress ? 'inline' : 'none'}}>
						<EZFormBase
