 ...
 * -----
 * Last Modified: Tuesday, 5th October 2021 2:07 pm
...
jest.mock('@siliconmtn/spacelibs-js/core/io/BaseHTTPService', () => {
  function getSelectData(url) {
    ...
    if (!value) {
      throw Error('Question data not found for ' + url);
    }
...
it('Renders a required MULTI question', () => {
  ...
  /** Click the dropdown */
  const input1 = screen.getAllByTestId('question-list')[0];
...
it('Renders a required CHOICE question', () => {
  ...
  const element = screen.getAllByTestId('question-list')[0];
  userEvent.click(element);
