 ...
 * -----
 * Last Modified: Wednesday, 21st July 2021 3:23 pm
...
export default function MuiTheme(props){
	...
	const contextValue = {
		...
		toggleTheme: () => {
			...
			document.body.style.backgroundColor = selectedThemeObject.palette.background.main;
		},
		theme: selectedThemeObject,
	};

	React.useEffect(() => {
		document.body.style.backgroundColor = selectedThemeObject.palette.background.paper;
	});

	return (
        <MuiThemeContext.Provider value={contextValue}>
            <ThemeProvider theme={createMuiTheme(contextValue.theme)}>
                {props.children}
            </ThemeProvider>
