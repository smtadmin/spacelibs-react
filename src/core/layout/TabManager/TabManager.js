 ...
 * -----
 * Last Modified: Thursday, 29th July 2021 1:41 pm
...
function TabManager(props) {
  ...
  function getTab(component, key, isDisabled, isExtra) {
    ...
    return (
      <Tab
        ...
        data-testid={`${component.toLowerCase()}-tab`}
  ...
  function getCompleteComponent(tabs, panels, defaultIndex) {
    return (
      <Tabs
        defaultIndex={props.selectedIndex != null ? undefined : defaultIndex}
        selectedIndex={props.selectedIndex}
        onSelect={props.onSelect}>
        <TabList className={['tab-list', props.variant, props.alignment]}>
          {tabs}
        </TabList>
        {panels}
      </Tabs>
  ...
  props.extraButtons?.forEach((item) => {
    tabs.push(
      getTab(item.tabComponent, 'tab-' + elementIndex++, item.isDisabled, true)
    );
    panels.push(
      getPanel(
